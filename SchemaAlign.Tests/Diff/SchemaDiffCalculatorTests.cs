...
public class SchemaDiffCalculatorTests
{
    ...
    [Fact]
    public void Calculate_DateTimeColumnsWithDifferentLengths_ShouldBeUnchanged()
    {
        var source = new DatabaseSchema();
        var target = new DatabaseSchema();

        var sourceTable = new TableSchema { Name = "SampleMaskedEntity", Schema = "dbo" };
        sourceTable.AddColumn(new ColumnSchema { Name = "Id", Type = StandardType.Int, IsPrimaryKey = true });
        sourceTable.AddColumn(new ColumnSchema { Name = "RecordedAt", Type = StandardType.DateTime, Length = null, IsNullable = false });
        source.AddTable(sourceTable);

        var targetTable = new TableSchema { Name = "SampleMaskedEntity", Schema = "dbo" };
        targetTable.AddColumn(new ColumnSchema { Name = "Id", Type = StandardType.Int, IsPrimaryKey = true });
        targetTable.AddColumn(new ColumnSchema { Name = "RecordedAt", Type = StandardType.DateTime, Length = 7, IsNullable = false });
        target.AddTable(targetTable);

        var diff = SchemaDiffCalculator.Calculate(source, target);

        diff.HasChanges.Should().BeFalse();
        var tableDiff = diff.FindTable("SampleMaskedEntity");
        tableDiff.Should().NotBeNull();
        tableDiff!.Kind.Should().Be(DiffKind.Unchanged);
        var colDiff = tableDiff.Columns.First(c => c.ColumnName == "RecordedAt");
        colDiff.Kind.Should().Be(DiffKind.Unchanged);
        colDiff.Changes.Should().Be(ChangeDetail.None);
    }

    [Fact]
    public void Calculate_ForeignKeysWithDifferentConstraintNames_MatchesByRelationTuple()
    {
        var source = new DatabaseSchema();
        var target = new DatabaseSchema();

        var sourceParent = new TableSchema { Name = "SampleParentEntity", Schema = "dbo" };
        sourceParent.AddColumn(new ColumnSchema { Name = "IdParentKey", Type = StandardType.String, Length = 36, IsPrimaryKey = true });
        source.AddTable(sourceParent);

        var sourceChild = new TableSchema { Name = "SampleChildEntity", Schema = "dbo" };
        sourceChild.AddColumn(new ColumnSchema { Name = "Id", Type = StandardType.Int, IsPrimaryKey = true });
        sourceChild.AddColumn(new ColumnSchema { Name = "CustomStatusCode", Type = StandardType.String, Length = 36 });
        sourceChild.AddForeignKey(new ForeignKeySchema
        {
            ConstraintName = "FK_SampleChildEntity_SampleParentEntity_CustomStatusCode",
            PrincipalTable = "SampleParentEntity",
            PrincipalColumn = "IdParentKey",
            DependentTable = "SampleChildEntity",
            DependentColumn = "CustomStatusCode",
            Cardinality = ForeignKeyCardinality.ManyToOne
        });
        source.AddTable(sourceChild);

        var targetParent = new TableSchema { Name = "SampleParentEntity", Schema = "dbo" };
        targetParent.AddColumn(new ColumnSchema { Name = "IdParentKey", Type = StandardType.String, Length = 36, IsPrimaryKey = true });
        target.AddTable(targetParent);

        var targetChild = new TableSchema { Name = "SampleChildEntity", Schema = "dbo" };
        targetChild.AddColumn(new ColumnSchema { Name = "Id", Type = StandardType.Int, IsPrimaryKey = true });
        targetChild.AddColumn(new ColumnSchema { Name = "CustomStatusCode", Type = StandardType.String, Length = 36 });
        targetChild.AddForeignKey(new ForeignKeySchema
        {
            ConstraintName = "FK_AutoGenerated_123",
            PrincipalTable = "SampleParentEntity",
            PrincipalColumn = "IdParentKey",
            DependentTable = "SampleChildEntity",
            DependentColumn = "CustomStatusCode",
        ...
        target.AddTable(targetChild);

        var diff = SchemaDiffCalculator.Calculate(source, target);

        var childTableDiff = diff.FindTable("SampleChildEntity");
        childTableDiff.Should().NotBeNull();
        childTableDiff!.ForeignKeys.Should().HaveCount(1);
        childTableDiff.ForeignKeys[0].Kind.Should().Be(DiffKind.Unchanged);
    }
